...
- Publisher: Canonical Ltd. (upstream), built by ML!PA Consulting GmbH from a patched source tree (TPM-related build changes, and a `constants.go` swapped in at build time from the private `ML-PA-Consulting-GmbH/snapd-snap` kit) then signed and republished through ML!PA's own snap store. Not Canonical's unmodified snapd binary.
- Licence: GPL-3.0-only (unchanged by the patching).
- Corresponding source: https://github.com/ML-PA-Consulting-GmbH/snapd, tag [`v5.7.1+up2.75.1-liot-image`](https://github.com/ML-PA-Consulting-GmbH/snapd/releases/tag/v5.7.1%2Bup2.75.1-liot-image) — the `-liot-image` release line is ML!PA's build flavour for L.IoT devices (this repository's own product family), distinct from other flavours in the same fork (e.g. `-ubuntu-image`). `constants.go` (store URLs, snap IDs, account keys) is generated per store and injected at build time from outside version control — every tag in this fork carries the same generic placeholder file, so that step is documented separately rather than pinned to a commit.
- Note: The snap daemon, patched for ML!PA's own store. Copyleft: the corresponding *modified* source must be made available to anyone who receives this binary, per GPL-3.0 §6 — the tag above plus the written offer above (**opensource@ml-pa.com**) for the build-time `constants.go` together satisfy that.
